...
class PredNet(Recurrent):
    ...
    def __init__(self, stack_sizes, R_stack_sizes,
        ...
        default_output_modes = ['prediction', 'error', 'all', 'representation', 'prediction_error',
                                'representation_and_error']
    ...
    def __compute_layer_shape(self, input_shape, layer_type=None, layer_num=None):
        ...
        if layer_type is None and self.output_mode == 'representation':
            layer_type = 'R'
        elif layer_type is None and self.output_mode == 'prediction_error':
            layer_type = 'E'
        elif layer_type is None:
            layer_type = self.output_layer_type

        if layer_num is None:
            layer_num = self.output_layer_num
        ...
        out_nb_row = int(input_shape[self.row_axis] / 2**layer_num)
        out_nb_col = int(input_shape[self.column_axis] / 2**layer_num)
    ...
    def unflatten_features(self, input_shape, feature_batch):
        ...
        for l in range(self.nb_layers):
            layer_shape = self.__compute_layer_shape(input_shape, layer_num=l)
    ...
    def compute_output_shape(self, input_shape):
        if self.output_mode == 'prediction':
            ...
            out_shape = (out_shape + self.nb_layers,)
        elif self.output_mode in ['representation', 'prediction_error']:
            out_shape = 0
            for l in range(self.nb_layers):
                layer_shape = self.__compute_layer_shape(input_shape, layer_num=l)
                flat_shape = np.prod(layer_shape)
    ...
    def build(self, input_shape):
        ...
        for c in sorted(self.conv_layers.keys()):
            for l in range(len(self.conv_layers[c])):
                ...
                if trainable:
                    layer_param_count = layer_utils.count_params(self.conv_layers[c][l].trainable_weights)
                    print(f'Layer {c}_{l} trainable params: {layer_param_count}')
                    trainable_param_count += layer_param_count
                    self.trainable_weights += self.conv_layers[c][l].trainable_weights
                else:
                    layer_param_count = layer_utils.count_params(self.conv_layers[c][l].non_trainable_weights)
                    print(f'Layer {c}_{l} non-trainable params: {layer_param_count}')
                    non_trainable_param_count += layer_param_count
                    self.non_trainable_weights += self.conv_layers[c][l].non_trainable_weights
    ...
    def step(self, a, states):
        ...
        if self.output_layer_type is None:
            ...
            elif self.output_mode == 'prediction_error':
                output = K.concatenate([K.batch_flatten(e[l]) for l in range(self.nb_layers)], axis=-1)
            else:
                for l in range(self.nb_layers):

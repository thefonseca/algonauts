...
def save_experiment_config(experiment_name, base_results_dir, config, dataset=None):   
    results_dir = get_create_results_dir(experiment_name, base_results_dir, dataset=dataset)
...
def get_config(FLAGS):
    ...
    if FLAGS.get('pretrained', None) is None and config.get('pretrained', None) is not None:
        model_suffix = '__' + config['pretrained']
    elif FLAGS.get('pretrained', None) is not None:
    ...
    if config['output_mode'] in ['prediction', 'representation', 'prediction_error']:
